...
def get_frame(discr, gen, dc_vars, device = None, discr_src = None):
    ...
    if mode.startswith("svgm"):
        ...
        print("discr.v1x", discr.v1x)
        print("std_v1x", std_v1x)
        print("discr.s1vx", discr.s1vx)
        print("std_s1vx", std_s1vx)
        print("q_args_stem", q_args_stem)
...
def get_ce_or_bce_loss(discr, dim_y: int, reduction: str="mean"):
    print("discr in get_ce_or_bce_loss", get_ce_or_bce_loss)
    print("dim_y:", dim_y)
    if dim_y == 1:
...
def add_ce_loss(lossobj, celossfn, ag):
    shrink_sup = ShrinkRatio(w_iter=ag.wsup_wdatum, decay_rate=ag.wsup_expo)
    print("*x_y_maybext_niter", *x_y_maybext_niter)
    def lossfn(*x_y_maybext_niter):
        loss = ag.wgen * lossobj(*x_y_maybext_niter)
        loss += celossfn(*x_y_maybext_niter[:2])
        if ag.wsup:
...
def ood_methods(discr, frame, ag, dim_y, cnbb_actv):
    if ag.mode not in MODES_GEN:
        print("ag.mode not in MODES_GEN")
        if ag.mode == "discr":
            print("ag.mode is discr")
        ...
        print("frame.logit_y1x_src", frame.logit_y1x_src)
        print("n_mc_q", ag.n_mc_q) #0
        print("ag.true_sup", ag.true_sup) #false
...
def main_stem(ag, ckpt, archtype, shape_x, dim_y,
    ...
    #if IS_OOD:   = len(tr_src_loader)
    #else: n_per_epk = max(len(tr_src_loader), len(tr_tgt_loader))
    
    #print("len(tr_src_loader)", len(tr_src_loader))
    #print("n_per_epk", n_per_epk)
    ...
    if gen is not None: 
        print("gen is not None-------------")
        pgc.collect_params(gen, frame)
    ...
    if ag.optim == "SGD":
        ...
        print("optim method is:", ag.optim)
        print("pgc.param_groups", pgc.param_groups)
    ...
    try:
        for epk in range(epk0, ag.n_epk+1):
            ...
            if IS_OOD:
                x, y = tr_src_loader[0], tr_src_loader[1]
                data_args = (x.to(device), y.to(device))
            else:
                (x, y), (xt, yt) = tr_src_loader[0], tr_src_loader[1]
